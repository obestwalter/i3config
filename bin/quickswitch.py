...
def get_workspaces():
    ...
    for ws in workspaces:
        # create_lookup_table sets value of all entries in the lookup table
...
def next_used(number):
    ...
    workspaces = sorted([int(ws) for ws in get_workspaces().keys()
                         if ws.isdecimal() and int(ws) > number])
...
def create_lookup_table(windows):
    ...
    for window in windows:
        ...
        if id_ is None:
            ...
            continue
        if any(name.startswith(i) for i in IGNORES):
